...
/* Change
  commented out fshader and vshader in string
  LightingTechnique() :
*/
...
bool LightingTechnique::Init(const char* vshader, const char* fshader)
{
    if (!Technique::Init()) {
        return false;
    }
    ...
    if (  !AddShader( GL_VERTEX_SHADER, readShaderSource(vshader) )  ) {
        return false;
    }

    // if (!AddShader(GL_FRAGMEeT_SHADER, pFS)) {
    if (  !AddShader( GL_FRAGMENT_SHADER, readShaderSource(fshader) )  ) {
        return false;
    }

    // std::cout << readShaderSource(fshader) << std::endl; 
