    ...
    for( unsigned int w = 0; w < objs.size(); w++ )
       {
        ...
        switch( w ){
            case 0:
                glUniformMatrix4fv(MVP_ID2, 1, GL_FALSE, glm::value_ptr(mvpBall));
                break;
            case 1:
                glUniformMatrix4fv(MVP_ID2, 1, GL_FALSE, glm::value_ptr(mvpWall));
                break;
            case 2:
                glUniformMatrix4fv(MVP_ID2, 1, GL_FALSE, glm::value_ptr(mvpFloor));
                break;
            case 3:
                glUniformMatrix4fv(MVP_ID2, 1, GL_FALSE, glm::value_ptr(mvpStart));
                break;
            case 4:
                glUniformMatrix4fv(MVP_ID2, 1, GL_FALSE, glm::value_ptr(mvpFinish));
                break;
            case 5:
        ...
        glVertexAttribPointer(loc_color2,   3, GL_FLOAT, GL_FALSE, sizeof(Point), (void*)offsetof(Point,color));
        ...
        glDisableVertexAttribArray(loc_normal2);
...
///// LOAD OBJ FILES ///////////////////////////////////////////////////////////////////
    ...
    // load the sphere
    const char* ballFile = "../Meshes/triSphere.obj";
    const char* ballName = "ball"; 
    Master ball( ballFile, ballName ); 

    // load the wall
    const char* wallFile = "../Meshes/Maze-NoFloor.obj";
    const char* wallName = "wall"; 
    Master wall( wallFile, wallName ); 

    // load the floor
    const char* floorFile = "../Meshes/Maze-Floor2.obj";
    const char* floorName = "floor"; 
    Master floor( floorFile, floorName );

    // load the start
    const char* startFile = "../Meshes/start.obj";
    const char* startName = "start"; 
    Master start( startFile, startName ); 

    // load the finish
    const char* finishFile = "../Meshes/finish.obj";
    const char* finishName = "finish"; 
    Master finish( finishFile, finishName ); 
    ...
    loc_color2 = glGetAttribLocation(program2, "vColor" );
